...
if __name__=="__main__":
    ...
    lr = 1
    ds = [40, 50, 60]
    regs = [1, 1e-1, 1e-2, 1e-3]
    data = np.load("../data/data_Y.pickle", allow_pickle=True)
    
    for target_benchmark in tqdm(benchmarks):
        print(f"****************** {target_benchmark} ******************")
        M = [data[k]['models'] for k in benchmarks if k!=target_benchmark]
        M = np.sort(list(reduce(set.intersection, map(set, M)))).tolist()
        Y = [data[k]['correctness'][[int(np.argmax(np.array(data[k]['models'])==m)) for m in M]] for k in benchmarks if k !=target_benchmark]
        Y = np.hstack(Y)
        
        irt_model = IRT(ds, device=device)
        irt_model.fit(Y, lr=lr, regs=regs, tol=tol, verbose=True)
        irt_model.save(f"../irt_models/{target_benchmark}")